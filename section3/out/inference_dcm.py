...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    """
    def original_img(slice_idx):
        original_slice = original_vol[slice_idx,:,:]
        
        # Flip, transpose and normalize numpy array to a matrix of values in the range of [0..255]
        nd_img = np.flip((original_slice/np.max(original_slice))*0xff).T.astype(np.uint8)
        
        # Create a PIL image from numpy array
        pil_img = Image.fromarray(nd_img, mode="L").convert("RGBA").resize(nd_img.shape)
        
        # Paste the PIL image into our main report image object (pimg)
        pimg.paste(pil_img, box=(10, 280))
        
        return pimg

    def masked_img(slice_idx):
        pred_slice = pred_vol[slice_idx, :, :]
        
        # Flip, transpose and normalize numpy array to a matrix of values in the range of [0..255]
        nd_img = np.flip((pred_slice / np.max(pred_slice)) * 0xff).T.astype(np.uint8)
    ...
    pil_orig = original_img(slice_nums[0])
    pil_pred = mask_img(slice_nums[0])
...
def get_series_for_inference(path):
    ...
    # <YOUR CODE HERE>
    #series_for_inference = [dcm for dcm in dicoms if dcm.Modality =="MR" ]
    
    #for dcm in range(dicoms):
    #    if (dcm.SeriesDescription == "HippoCrop"):
    #        series_for_inference.append(dcm)
...
if __name__ == "__main__":
    ...
    print(sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True))
    #study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)[0]
    study_dir = glob.glob(sys.argv[1] + "/*HCrop*/")[0]
    ...
    report_save_path = r"/home/workspace/out/report_study3m.dcm"
    ...
    os_command('storescu 127.0.0.1 4242 -v -aec HIPPOAI /home/workspace/src/report_study3m.dcm')
